		...
		mojangCall.enqueue(object : Callback<Player> {
			override fun onResponse(call: Call<Player>, response: Response<Player>) {
				...
				if (response.isSuccessful) {
					handleMojangCallResponse(body)
				} else {
					hideLoading()
					binding.buttonMainSearch.isEnabled = true
					Log.e(TAG, "Mojang API error: ${response.code()} - ${response.message()}")
					Toast.makeText(this@MainActivity, "Bad response from Mojang API", Toast.LENGTH_SHORT).show()
				}
			}
		...
		hypixelCall.enqueue(object : Callback<PlayerData> {
			override fun onResponse(call: Call<PlayerData>, response: Response<PlayerData>) {
				...
				if (response.isSuccessful && body != null) {
					loadedStatistics = Statistics.from(this@MainActivity, body)
					if (loadedStatistics == null) {
						isLoading = false
						hideLoading()
						binding.buttonMainSearch.isEnabled = true
						Log.d(TAG, "Hypixel API: No Bedwars data found")
						return
					}
					...
					Toast.makeText(this@MainActivity, "Bad response from Hypixel API", Toast.LENGTH_SHORT).show()
				}
			}
